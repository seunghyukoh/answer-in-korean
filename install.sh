...
codex_install() {
  if [[ -e $agent_toml ]] && ! head -1 "$agent_toml" 2>/dev/null | grep -qF "$marker"; then
    die "$agent_toml exists and was not generated by this script; move it away first (nothing was changed)"
  fi
  # Re-adding the same source is a no-op; a different source fails, so re-register from this one.
  if ! cx plugin marketplace add "$source" >/dev/null 2>&1; then
    cx plugin marketplace remove "$market" >/dev/null 2>&1 || true
    cx plugin marketplace add "$source" >/dev/null || die "codex plugin marketplace add $source failed"
    echo "codex: marketplace $market re-registered from $source"
  fi
